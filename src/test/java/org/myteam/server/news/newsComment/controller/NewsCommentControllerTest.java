...
class NewsCommentControllerTest extends ControllerTestSupport {
	...
	void recommendTest() throws Exception {
		...
		// when // then
		mockMvc.perform(
				patch("/api/news/comment/recommend/{newsCommentId}", 1)
					.with(csrf())
			)
			.andDo(print())
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.status").value(SUCCESS.name()))
			.andExpect(jsonPath("$.msg").value("뉴스 댓글 추천 성공"));
	}
	...
	void recommendCancelTest() throws Exception {
		...
		// when // then
		mockMvc.perform(
				delete("/api/news/comment/recommend/{newsCommentId}", 1)
					.with(csrf())
			)
			.andDo(print())
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.status").value(SUCCESS.name()))
			.andExpect(jsonPath("$.msg").value("뉴스 댓글 추천 삭제 성공"));
	}
}
