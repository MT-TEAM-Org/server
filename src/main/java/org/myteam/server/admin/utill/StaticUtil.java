...
import org.myteam.server.member.entity.QMemberAccess;
import org.myteam.server.report.domain.QReport;
...
import static org.myteam.server.member.entity.QMemberAccess.*;
import static org.myteam.server.report.domain.QReport.*;
...
public class StaticUtil {
    ...
    public static Predicate between_static_time(LocalDateTime static_end_time, LocalDateTime static_start_time, Object target){
        ...
        switch(name){
            case "Report"->{return report.createDate.goe(static_end_time).and(report.createDate.lt(static_start_time));}
            case "Member"->{return member.createDate.goe(static_end_time).and(member.createDate.lt(static_start_time));}
            case "Board"->{return board.createDate.goe(static_end_time).and(board.createDate.lt(static_start_time));}
            case "Comment"->{return comment1.createDate.goe(static_end_time).and(comment1.createDate.lt(static_start_time));}
            case "Improvement"->{return improvement.createDate.goe(static_end_time).and(improvement.createDate.lt(static_start_time));}
            case "Inquiry"->{return inquiry.createdAt.goe(static_end_time).and(inquiry.createdAt.lt(static_start_time));}
            case "MemberAccess"->{return memberAccess.accessTime.goe(static_end_time).and(memberAccess.accessTime.lt(static_start_time));}
            default ->{ return null;}
